...
def send_diagnostic_output(url: str, payload: dict[str, Any], timeout: float = 5.0) -> dict[str, Any]:
    ...
    try:
        ...
        # LINEAJE: enforce() `response` at api->agent post_tool — scan flagged AI_SKILL_SEC_001 (Do not allow use of malicious skills). Mask/block; do not remove without review. site_id='site:sha256:75533f323a61c3a23dec9b0b5fbb8db3879972fd771d852d8addb176af814d0e'
        _gr_client = _lineaje_load_gr_client()
        ...
        try:
            response = _gr_client.enforce(_gr_site, response, content_type='application/json', variable_name='response', source_file=__file__, before_line=129)
        except _gr_client.GuardrailUnavailableError:
            pass
