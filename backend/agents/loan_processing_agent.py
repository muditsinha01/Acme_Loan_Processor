...
class LoanProcessingAgent(AcmeLoanAgentFramework):
    ...
    async def call_agent_model(self, user_message: str, file_summary: str) -> str:
        ...
        _gr_site = _gr_client.SiteDescriptor(site_id='site:sha256:8c43a45fa64193e6c2dccfb1c14edcb28197739a7513ba0362189419ddd1795a', phase='pre_model', boundary={'source': 'agent_message', 'sink': 'model'}, candidate_policies=[], fail_mode='BLOCK', source_type='agent', destination_type='llm')
        try:
            _lineaje_messages = await __import__('asyncio').to_thread(lambda: _gr_client.enforce(_gr_site, _lineaje_messages, content_type='application/json', variable_name='_lineaje_messages', source_file=__file__, before_line=29))
    ...
    async def handle(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        _gr_client = _lineaje_load_gr_client()
        _gr_site = _gr_client.SiteDescriptor(site_id='site:sha256:9a9ccef0ab9d02e67bc618e1d8b5c69e7027d86e8438dc2c7db423a766187a31', phase='pre_model', boundary={'source': 'agent_message', 'sink': 'model'}, candidate_policies=[], fail_mode='BLOCK', source_type='agent', destination_type='llm')
        try:
            user_message = await __import__('asyncio').to_thread(lambda: _gr_client.enforce(_gr_site, user_message, content_type='application/json', variable_name='user_message', source_file=__file__, before_line=49))
        except _gr_client.GuardrailUnavailableError:
            pass
        except PermissionError:
            raise
