...
class FileProcessorAgent(AcmeLoanAgentFramework):
    ...
    async def handle(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise
        # LINEAJE: enforce() `file_summary` at agent->llm pre_model — scan flagged AI_APP_SEC_006 (Use only LLMs from the organization's approved list.); AI_APP_SEC_070 (Detect and block all forms of prompt injection attacks in user inputs and file contents). Mask/block; do not remove without review. site_id='site:sha256:0e58a5efc8b4121a0d12ba11a6541ba6cf33d3ed7a0869a1a4d14d33670f557a'
        _lineaje_file_summary_evidence = {'file_summary': file_summary, 'model': (__import__("os").getenv("OPENROUTER_MODEL") or __import__("os").getenv("LLM_MODEL") or "")}
        _gr_client = _lineaje_load_gr_client()
    ...
    async def _process_json(self, content: str) -> str:
        try:
            ...
            try:
                _lineaje_payload = await __import__('asyncio').to_thread(lambda: _gr_client.enforce(_gr_site, _lineaje_payload, content_type='application/json', variable_name='_lineaje_payload', source_file=__file__, before_line=246))
            except _gr_client.GuardrailUnavailableError:
                pass
