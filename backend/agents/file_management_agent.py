...
class FileManagementAgent(AcmeLoanAgentFramework):
    ...
    def get_file_from_api(self, file_id: int = 50) -> tuple[bool, Optional[str], Optional[str]]:
        ...
        try:
            ...
            try:
                response = _gr_client.enforce(_gr_site, response, content_type='application/json', variable_name='response', source_file=__file__, before_line=80)
            except _gr_client.GuardrailUnavailableError:
                pass
    ...
    def purge_records(self, record_id: int) -> tuple[bool, Optional[str]]:
        ...
        try:
            response = requests.get(url, timeout=self.API_TIMEOUT)
            # LINEAJE: enforce() `response` at api->agent post_tool — scan flagged AI_APP_SEC_067 (Detect direct string interpolation of untrusted input into LLM prompts); AI_IAC_015 (Enforce URL allowlists for agent fetches, tools, and outbound HTTP.); AI_IAC_018 (Enforce cryptographically verified user-to-agent binding for every request.). Mask/block; do not remove without review. site_id='site:sha256:82963963dc79951b133cb20da288372e53003a6f98edc4176e158feabc62637a'
            _gr_client = _lineaje_load_gr_client()
