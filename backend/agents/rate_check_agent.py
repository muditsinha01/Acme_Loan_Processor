...
class RateCheckAgent(AcmeLoanAgentFramework):
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        # LINEAJE: enforce() `metadata` at agent->user_interface data_egress — scan flagged AI_IAC_015 (Enforce URL allowlists for agent fetches, tools, and outbound HTTP.); AI_IAC_018 (Enforce cryptographically verified user-to-agent binding for every request.); AI_IAC_023 (Chatbot and AI interfaces must disclose AI identity to the user). Mask/block; do not remove without review. site_id='site:sha256:0df19c3582f994fd226b63d4b3c50449cc5e39c6b94467c9dc70cc89cfdf4dcd'
    ...
    async def call_agent_model(self, user_message: str) -> str:
        ...
        # LINEAJE: enforce() `_lineaje_payload` at agent->log log_emit — scan flagged AI_IAC_015 (Enforce URL allowlists for agent fetches, tools, and outbound HTTP.); AI_IAC_018 (Enforce cryptographically verified user-to-agent binding for every request.); AI_IAC_023 (Chatbot and AI interfaces must disclose AI identity to the user). Mask/block; do not remove without review. site_id='site:sha256:1c5ab84b3fc1af40ad31a390eda6314849d3834d077e69f40bbf40f23c96917a'
        _gr_client = _lineaje_load_gr_client()
