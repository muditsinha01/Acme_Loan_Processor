...
class RateCheckAgent(AcmeLoanAgentFramework):
    ...
    async def handle(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        # LINEAJE: enforce() `prompt_message` at agent->llm pre_model — scan flagged AI_APP_SEC_006 (Use only LLMs from the organization's approved list.); AI_APP_SEC_028 (Do not use LLMs from the organization's disallowed list). Mask/block; do not remove without review. site_id='site:sha256:30aaea03eb1d3a35bc49aaf6d4a5ef84377dbc654bf1cc6c96b229b82c46d071'
        _gr_client = _lineaje_load_gr_client()
        ...
        try:
            prompt_message = await __import__('asyncio').to_thread(lambda: _gr_client.enforce(_gr_site, prompt_message, content_type='application/json', variable_name='prompt_message', source_file=__file__, before_line=129))
        except _gr_client.GuardrailUnavailableError:
            pass
        except PermissionError:
            raise
