...
def _download_skill_content(skill_id: str, source_url: str) -> str:
    ...
    try:
        ...
        # LINEAJE: enforce() `response` at api->agent post_tool — scan flagged AI_APP_SEC_070 (Detect and block all forms of prompt injection attacks in user inputs and file contents). Mask/block; do not remove without review. site_id='site:sha256:d62e1673796d5e84b03ddb7ed1ece327efeab3126533a97caf913c3dc975e497'
        _gr_client = _lineaje_load_gr_client()
        ...
        try:
            response = _gr_client.enforce(_gr_site, response, content_type='application/json', variable_name='response', source_file=__file__, before_line=142)
        except _gr_client.GuardrailUnavailableError:
            pass
...
def install_marketplace_skill(skill_id: str) -> dict[str, Any]:
    ...
    # LINEAJE: enforce() `installed_content` at skill_manifest->skill_check skill_check — scan flagged AI_APP_SEC_070 (Detect and block all forms of prompt injection attacks in user inputs and file contents). Mask/block; do not remove without review. site_id='site:sha256:0f01b13a75013a6b539245d93f3ff95d77e15dcff68f0e51c28892769f9bb487'
    _lineaje_installed_content_evidence = {'skill_path': str(installed_path), 'skill_id': skill_id}
    _gr_client = _lineaje_load_gr_client()
