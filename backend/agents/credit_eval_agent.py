...
class CreditEvalAgent(AcmeLoanAgentFramework):
    ...
    async def call_agent_model(self, combined_context: str) -> str:
        ...
        try:
            _lineaje_messages = await __import__('asyncio').to_thread(lambda: _gr_client.enforce(_gr_site, _lineaje_messages, content_type='application/json', variable_name='_lineaje_messages', source_file=__file__, before_line=70))
        except _gr_client.GuardrailUnavailableError:
            pass
        except PermissionError:
            raise
    ...
    async def handle(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            safe_combined_context = await __import__('asyncio').to_thread(lambda: _gr_client.enforce(_gr_site, safe_combined_context, content_type='application/json', variable_name='safe_combined_context', source_file=__file__, before_line=107))
        except _gr_client.GuardrailUnavailableError:
            pass
        except PermissionError:
            raise
