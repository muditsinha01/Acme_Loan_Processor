...
blocked may still re-POST (model settings can change).
"""
...
# mcp_call, tool_to_user, …). ContextVar so concurrent requests on other
# tasks are not poisoned. A process-wide copy also covers
# ``asyncio.to_thread`` (ContextVar mutations in the worker do not copy
# back to the parent task) so a blocked LLM does not still POST PII/MCP
# sites. The blocking site_id may still re-POST.
_PROCESS_BLOCK: contextvars.ContextVar[dict[str, Any] | None] = contextvars.ContextVar(
    "gr_process_block", default=None,
)
_PROCESS_BLOCK_LOCK = threading.Lock()
_PROCESS_BLOCK_SHARED: dict[str, Any] | None = None
...
def _reset_runtime_caches() -> None:
    ...
    _PROCESS_BLOCK.set(None)
    with _PROCESS_BLOCK_LOCK:
        _PROCESS_BLOCK_SHARED = None
...
def _record_process_block(site_id: str, warning: str, policy_id: str = "") -> None:
    ...
    if _PROCESS_BLOCK.get() is None:
        _PROCESS_BLOCK.set(payload)
    with _PROCESS_BLOCK_LOCK:
        if _PROCESS_BLOCK_SHARED is None:
            _PROCESS_BLOCK_SHARED = payload


def _current_process_block() -> dict[str, Any] | None:
    prior = _PROCESS_BLOCK.get()
    if prior:
        return prior
    with _PROCESS_BLOCK_LOCK:
        return _PROCESS_BLOCK_SHARED
